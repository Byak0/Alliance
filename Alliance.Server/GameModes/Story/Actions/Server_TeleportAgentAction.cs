...
namespace Alliance.Server.GameModes.Story.Actions
{
	public class Server_TeleportAgentAction : TeleportAgentAction
	{
		public override void Execute()
		{
			...
			Mission.Current.GetNearbyAgents(StartingZone.GlobalPosition.AsVec2, StartingZone.Radius, agents);
			agents.RemoveAll(agent => !IsValidTarget(agent));
			foreach (Agent agent in agents)
			{
				Vec3 destination = CoreUtils.GetRandomPositionWithinRadius(DestinationZone.GlobalPosition, DestinationZone.Radius);
		...
		public bool IsValidTarget(Agent agent)
		{
			...
			if (agent.Position.Distance(StartingZone.GlobalPosition) > StartingZone.Radius) return false;
			// Other checks
