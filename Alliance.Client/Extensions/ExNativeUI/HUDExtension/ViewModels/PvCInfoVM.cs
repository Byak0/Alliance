...
namespace Alliance.Client.Extensions.ExNativeUI.HUDExtension.ViewModels
{
	public class PvCInfoVM : ViewModel
	{
		...
		internal void Tick(float dt)
		{
			...
			{
				int count = Mission.Current.AttackerTeam.ActiveAgents.Where(agent => agent.IsHuman).Count();
				int count2 = Mission.Current.DefenderTeam.ActiveAgents.Where(agent => agent.IsHuman).Count();
