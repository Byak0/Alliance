    ...
    public class SaeBehavior : MissionView, IUseKeyBinder
    {
        ...
        private void WhenDynamicCreateMarkerActionTriggered()
        {
            if (Input.IsKeyDown(InputKey.LeftAlt))
            {
                ...
                if (saeVirtualCursorManager.CursorMarker != null && DateTime.Now.Ticks - lastTimeMakerPosed.Ticks >= 1000000 && Input.IsKeyDown(fastCreateMarkerIK.KeyboardKey.InputKey))
                {
                    lastTimeMakerPosed = DateTime.Now;
                    SendToServerEligibleMarkersToSpawnInZone();
                }
                else if (saeVirtualCursorManager.CursorMarker != null && DateTime.Now.Ticks - lastTimeMakerPosed.Ticks >= 1000000 && Input.IsKeyDown(fastDeleteMarkerIK.KeyboardKey.InputKey))
                {
                    lastTimeMakerPosed = DateTime.Now;
        ...
        private void WhenCreateMarkerActionTriggered()
        {
            ...
            if (DateTime.Now.Ticks - lastTimeMakerPosed.Ticks >= 1500000 && Input.IsKeyDown(createMarkerIK.KeyboardKey.InputKey))
            {
                lastTimeMakerPosed = DateTime.Now;
                SendToServerToCreateMarker();
            }
        ...
        private void WhenDeleteMarkerActionTriggered()
        {
            ...
            if (Input.IsKeyPressed(deleteAllMarkerIK.KeyboardKey.InputKey))
            {
