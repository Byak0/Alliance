...
namespace Alliance.Common.Core.KeyBinder
{
	...
	public static class KeyBinder
	{
		...
		/// </summary>
		public static void Initialize(List<Assembly> assembly)
		{
			if (DontUseKeyBinder) return;

			foreach (Assembly asm in assembly) AutoRegister(asm);
		...
		private static void AutoRegister(Assembly assembly)
		{
			try
			{
				// Find all types that implement IUseKeyBinder and are not abstract
				IEnumerable<Type> binderTypes = assembly.GetTypes().Where(t => t.GetInterfaces().Contains(typeof(IUseKeyBinder)) && !t.IsAbstract).ToList();
