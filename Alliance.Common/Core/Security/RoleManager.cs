...
namespace Alliance.Common.Core.Security
{
	...
	public class RoleManager
	{
		...
		public void UpdatePlayersRoles(int fieldIndex, Player player, bool remove = false)
		{
			...
			int beforeCount = oldList.Count;
			if (remove || oldList.Contains(player))
			{
				int deletion = oldList.RemoveAll(p => p.Id == player.Id);
				Log($"Alliance Role Manager - {player.Name} ({player.Id}) {(deletion > 0 ? "removed from " : "not found in ")}{RolesFields[fieldIndex].Name}.", LogLevel.Debug);
			}
			if (!remove)
			{
				oldList.Add(player);
				Log("Alliance Role Manager - " + player.Name + " (" + player.Id + ")" + " added to " + RolesFields[fieldIndex].Name + ".", LogLevel.Debug);
			}
			Log($"Nb players with role {RolesFields[fieldIndex].Name} : {oldList.Count} (was {beforeCount})", LogLevel.Debug);
		}
